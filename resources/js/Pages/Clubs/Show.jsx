                ...
                <div className="flex items-center justify-between w-full">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">{club.short_name} • {club.country}</p>
                        </div>
                    </div>
                    {isOwner && (
                        <Link 
                            href={route('clubs.edit', club.id)}
                            className="bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all border border-slate-700"
                        >
                            Einstellungen
                        </Link>
                    )}
        ...
        >
            ...
            <div className="py-8">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                    ...
                    {/* Navigation Tabs */}
                    <div className="flex items-center space-x-1 bg-slate-900 p-1 rounded-xl border border-slate-800 w-fit">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
                                    <h4 className="text-white text-[10px] font-black uppercase tracking-widest mb-6 flex items-center">
                                        <Info size={14} weight="bold" className="mr-2 text-slate-500" /> Vereinsinfos
                                    </h4>
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-slate-800/20">
                                                <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest">Spieler</th>
                                                <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest text-center">Pos</th>
                                                <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest text-center">Alter</th>
                                                <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest text-center">Stärke</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-800">
                                            {players.slice(0, 10).map(player => (
                                                <tr key={player.id} className="hover:bg-slate-800/30 transition-colors cursor-pointer group" onClick={() => router.get(route('players.show', player.id))}>
                                                    ...
                                                    <td className="px-6 py-4 text-center">
                                                        <span className="text-[10px] font-black bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded border border-slate-700">{player.position}</span>
                                                    </td>
