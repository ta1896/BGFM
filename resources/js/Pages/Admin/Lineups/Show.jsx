            ...
            <div className="space-y-6 max-w-4xl">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-[var(--text-muted)] text-sm mt-1">Formation: <span className="text-indigo-400 font-bold">{lineup.formation}</span></p>
                    </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        ...
                        <div key={m.label} className="sim-card p-4 text-center">
                            <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] mb-1">{m.label}</p>
                ...
                <div className="sim-card overflow-hidden">
                    <div className="p-4 border-b border-[var(--border-pillar)] flex items-center gap-2">
                    ...
                    <div className="divide-y divide-slate-800/50">
                        {lineup.players?.map(player => (
                            <div key={player.id} className="flex items-center gap-4 px-5 py-3 hover:bg-[var(--bg-content)]/20 transition">
                                <img src={player.photo_url} className="h-9 w-9 rounded-lg object-cover border border-[var(--border-pillar)] bg-[var(--bg-pillar)]" alt="" />
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-bold text-white truncate">{player.full_name}</p>
                                    <p className="text-[10px] text-[var(--text-muted)]">{player.position} · {player.pivot?.pitch_position || '—'}</p>
                                </div>
                                <span className="text-xl font-black text-white">{player.overall}</span>
                            </div>
                        ))}
                        {!lineup.players?.length && (
                            <p className="px-5 py-8 text-center text-[var(--text-muted)] italic text-sm">Keine Spieler zugeordnet.</p>
                        )}
                    </div>
                </div>
            </div>
