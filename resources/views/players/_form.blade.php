...
<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
    ...
    <div>
        ...
        <input class="sim-input" id="first_name" name="first_name" type="text" value="{{ old('first_name', $player->first_name ?? '') }}" required>
        <x-input-error :messages="$errors->get('first_name')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="last_name">Nachname</label>
        <input class="sim-input" id="last_name" name="last_name" type="text" value="{{ old('last_name', $player->last_name ?? '') }}" required>
    ...
    <div>
        ...
        <input class="sim-input" id="age" name="age" type="number" min="15" max="45" value="{{ old('age', $player->age ?? 22) }}" required>
        <x-input-error :messages="$errors->get('age')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="overall">Overall</label>
        <input class="sim-input" id="overall" name="overall" type="number" min="1" max="99" value="{{ old('overall', $player->overall ?? 60) }}" required>
        <x-input-error :messages="$errors->get('overall')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="pace">Tempo</label>
        <input class="sim-input" id="pace" name="pace" type="number" min="1" max="99" value="{{ old('pace', $player->pace ?? 60) }}" required>
        <x-input-error :messages="$errors->get('pace')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="shooting">Schuss</label>
        <input class="sim-input" id="shooting" name="shooting" type="number" min="1" max="99" value="{{ old('shooting', $player->shooting ?? 60) }}" required>
        <x-input-error :messages="$errors->get('shooting')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="passing">Pass</label>
        <input class="sim-input" id="passing" name="passing" type="number" min="1" max="99" value="{{ old('passing', $player->passing ?? 60) }}" required>
        <x-input-error :messages="$errors->get('passing')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="defending">Defensive</label>
        <input class="sim-input" id="defending" name="defending" type="number" min="1" max="99" value="{{ old('defending', $player->defending ?? 60) }}" required>
        <x-input-error :messages="$errors->get('defending')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="physical">Physis</label>
        <input class="sim-input" id="physical" name="physical" type="number" min="1" max="99" value="{{ old('physical', $player->physical ?? 60) }}" required>
        <x-input-error :messages="$errors->get('physical')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="stamina">Ausdauer</label>
        <input class="sim-input" id="stamina" name="stamina" type="number" min="1" max="100" value="{{ old('stamina', $player->stamina ?? 80) }}" required>
        <x-input-error :messages="$errors->get('stamina')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="morale">Moral</label>
        <input class="sim-input" id="morale" name="morale" type="number" min="1" max="100" value="{{ old('morale', $player->morale ?? 60) }}" required>
        <x-input-error :messages="$errors->get('morale')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="market_value">Marktwert (EUR)</label>
        <input class="sim-input" id="market_value" name="market_value" type="number" min="0" step="0.01" value="{{ old('market_value', $player->market_value ?? 1000000) }}" required>
        <x-input-error :messages="$errors->get('market_value')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="salary">Gehalt (EUR)</label>
        <input class="sim-input" id="salary" name="salary" type="number" min="0" step="0.01" value="{{ old('salary', $player->salary ?? 15000) }}" required>
