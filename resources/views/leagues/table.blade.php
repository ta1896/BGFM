...
<x-app-layout>
    <div class="space-y-6">
                ...
                @if ($activeCompetitionSeason)
                    <div class="mt-2 flex items-center gap-3">
                        <div class="bg-slate-800 rounded px-2 py-1 border border-slate-700">
                            <img class="h-6 w-auto" src="{{ $activeCompetitionSeason->competition->logo_url }}"
                                alt="{{ $activeCompetitionSeason->competition->name }}">
                        </div>
                        <span class="text-lg text-slate-300 font-medium">{{ $activeCompetitionSeason->competition->name }}
                            <span class="text-slate-500 mx-1">|</span> {{ $activeCompetitionSeason->season->name }}</span>
                    </div>
                @endif
            </div>
            ...
            <form method="GET" action="{{ route('league.table') }}" class="flex flex-wrap items-center gap-3">
                ...
                <div class="relative group">
                    <select name="competition_id"
                        class="sim-input pl-4 pr-10 py-2 text-sm bg-slate-900/80 backdrop-blur-md border-slate-700 focus:border-cyan-500 rounded-lg appearance-none cursor-pointer min-w-[200px]"
                    ...
                    <div
                        class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none text-slate-400 group-hover:text-cyan-400 transition-colors">
                ...
                <div class="relative group">
                    <select name="season_id"
                        class="sim-input pl-4 pr-10 py-2 text-sm bg-slate-900/80 backdrop-blur-md border-slate-700 focus:border-cyan-500 rounded-lg appearance-none cursor-pointer min-w-[120px]"
                        onchange="this.form.submit()">
        ...
        <div
            class="sim-card py-3 px-4 flex flex-col xl:flex-row xl:items-center justify-between gap-4 bg-slate-900/60 backdrop-blur-md">
            <!-- Tabs -->
            <div class="flex flex-wrap gap-1 p-1 bg-slate-900/50 rounded-lg border border-slate-700/50">
                @foreach (['Gesamt', 'Hinrunde', 'Rueckrunde', 'Heim', 'Auswaerts', 'Form'] as $tab)
                    <button type="button"
                        class="px-3 py-1.5 rounded-md text-xs font-bold uppercase tracking-wide transition-all {{ $loop->first ? 'bg-cyan-500 text-white shadow-lg shadow-cyan-500/20' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                        {{ $tab }}
                    </button>
                @endforeach
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3 text-[10px] font-bold uppercase tracking-wider">
                <span
                    class="flex items-center gap-1.5 px-2 py-1 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span> Super League
                </span>
                <span
                    class="flex items-center gap-1.5 px-2 py-1 rounded bg-sky-500/10 text-sky-400 border border-sky-500/20">
                    <span class="w-1.5 h-1.5 rounded-full bg-sky-400"></span> SL-Quali
                </span>
                <span
                    class="flex items-center gap-1.5 px-2 py-1 rounded bg-rose-500/10 text-rose-400 border border-rose-500/20">
            ...
            <div class="sim-card p-0 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr
                                class="border-b border-slate-700/50 text-xs font-bold uppercase tracking-wider text-slate-400 bg-slate-900/40">
                        ...
                        <tbody class="divide-y divide-slate-700/50">
                            @foreach ($table as $index => $row)
                                                    ...
                                                    @endphp
                                <tr
                                                        class="group hover:bg-white/[0.02] transition-colors {{ $isOwned ? 'bg-cyan-500/5 hover:bg-cyan-500/10' : '' }}">
                                                        <td class="px-4 py-3 text-center">
                                                            <div
                                                                class="w-8 h-8 rounded mx-auto flex items-center justify-center font-bold text-sm border border-transparent {{ $rankBg }} {{ $rankClass }}">
                                                                {{ $index + 1 }}
                                                            </div>
                                                        </td>
                                                        <td class="px-4 py-3">
                                                            <div class="flex items-center gap-3">
                                                                <div
                                                                    class="w-8 h-8 rounded bg-slate-800 p-1 border border-slate-700 flex items-center justify-center shrink-0">
                                                                    <img src="{{ optional($row->club)->logo_url }}"
                                                                        alt="{{ optional($row->club)->name }}" class="max-w-full max-h-full">
                                                                </div>
                                                                <a href="{{ route('clubs.show', $row->club_id) }}"
                                                                    class="font-bold text-white group-hover:text-cyan-400 transition-colors {{ $isOwned ? 'text-cyan-400' : '' }} hover:underline">{{ optional($row->club)->name }}</a>
                                                                @if($isOwned)
                                                                    <span
                                                                        class="text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-cyan-500/20 text-cyan-400 border border-cyan-500/30">Du</span>
                                                                @endif
                                                            </div>
                                                        </td>
                                                        ...
                                                        <td class="px-4 py-3 text-center font-mono text-slate-300">
                                                            {{ $row->goals_for }}:{{ $row->goals_against }}</td>
                                                        <td
                                                            class="px-4 py-3 text-center font-mono {{ $row->goal_diff > 0 ? 'text-emerald-400' : ($row->goal_diff < 0 ? 'text-rose-400' : 'text-slate-400') }}">
