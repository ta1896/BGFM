    ...
    <div x-data="{ activeTab: 'overview' }" class="space-y-6">
        ...
        <div x-show="activeTab === 'overview'" x-transition.opacity class="space-y-6">
            ...
            @if($club->achievements->isNotEmpty())
                <section class="sim-card p-5 border-b-4 border-b-amber-500/50">
                    <p class="sim-section-title flex items-center gap-2 text-amber-400">
                    ...
                    <div class="mt-4 flex flex-wrap gap-4">
                        @foreach($club->achievements as $achievement)
                            <div class="flex items-center gap-3 bg-slate-800/60 p-3 rounded-lg border border-amber-500/20 shadow-lg shadow-amber-900/10 group hover:border-amber-500/40 transition-all">
                                <div class="w-10 h-10 bg-amber-500/10 rounded-full flex items-center justify-center text-amber-500 group-hover:scale-110 transition-transform">
                                    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                </div>
                                <div>
                                    <p class="text-sm font-bold text-white leading-tight">{{ $achievement->title }}</p>
                                    <p class="text-[10px] text-slate-400 uppercase tracking-wider">{{ $achievement->achieved_at->format('Y') }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
