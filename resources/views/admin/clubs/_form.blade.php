...
<div class="grid gap-4 md:grid-cols-2">
    ...
    <div>
        ...
        <input class="sim-input" id="name" name="name" type="text" value="{{ old('name', $club->name ?? '') }}"
            required>
        <x-input-error :messages="$errors->get('name')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="short_name">Kurzname</label>
        <input class="sim-input" id="short_name" name="short_name" type="text"
    ...
    <div>
        ...
        @if (!empty($club?->logo_path))
            <div class="mt-2 flex items-center gap-2 text-xs text-slate-400">
                <img class="sim-avatar sim-avatar-md" src="{{ $club->logo_url }}" alt="Club Logo">
    ...
    <div>
        ...
        <input class="sim-input" id="country" name="country" type="text"
            value="{{ old('country', $club->country ?? 'Deutschland') }}" required>
        <x-input-error :messages="$errors->get('country')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="league">Liga</label>
        <input class="sim-input" id="league" name="league" type="text"
            value="{{ old('league', $club->league ?? 'Amateurliga') }}" required>
        <x-input-error :messages="$errors->get('league')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="founded_year">Gruendungsjahr</label>
        <input class="sim-input" id="founded_year" name="founded_year" type="number"
    ...
    <div>
        ...
        <input class="sim-input" id="reputation" name="reputation" type="number" min="1" max="99"
            value="{{ old('reputation', $club->reputation ?? 50) }}" required>
        <x-input-error :messages="$errors->get('reputation')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="fan_mood">Fan Stimmung</label>
        <input class="sim-input" id="fan_mood" name="fan_mood" type="number" min="1" max="100"
            value="{{ old('fan_mood', $club->fan_mood ?? 50) }}" required>
    ...
    <div>
        ...
        <select class="sim-select" id="season_objective" name="season_objective">
            ...
            <option value="title" @selected(old('season_objective', $club->season_objective ?? 'mid_table') === 'title')>
    ...
    <div>
        ...
        <input class="sim-input" id="budget" name="budget" type="number" min="0" step="0.01"
            value="{{ old('budget', $club->budget ?? 500000) }}" required>
        <x-input-error :messages="$errors->get('budget')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="coins">Coins</label>
        <input class="sim-input" id="coins" name="coins" type="number" min="0" step="1"
            value="{{ old('coins', $club->coins ?? 0) }}">
        <x-input-error :messages="$errors->get('coins')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="wage_budget">Gehaltsbudget</label>
        <input class="sim-input" id="wage_budget" name="wage_budget" type="number" min="0" step="0.01"
            value="{{ old('wage_budget', $club->wage_budget ?? 250000) }}" required>
        <x-input-error :messages="$errors->get('wage_budget')" class="mt-1" />
    </div>

    <!-- Rivals -->
    <div>
        <label class="sim-label" for="rival_id_1">Erzrivale 1</label>
        <select class="sim-select" id="rival_id_1" name="rival_id_1">
            <option value="">-- Kein Rivale --</option>
            @foreach ($clubs as $rClub)
                <option value="{{ $rClub->id }}" @selected(old('rival_id_1', $club->rival_id_1 ?? '') == $rClub->id)>
                    {{ $rClub->name }}
                </option>
            @endforeach
        </select>
        <x-input-error :messages="$errors->get('rival_id_1')" class="mt-1" />
    </div>
    <div>
        <label class="sim-label" for="rival_id_2">Erzrivale 2</label>
        <select class="sim-select" id="rival_id_2" name="rival_id_2">
            <option value="">-- Kein Rivale --</option>
            @foreach ($clubs as $rClub)
                <option value="{{ $rClub->id }}" @selected(old('rival_id_2', $club->rival_id_2 ?? '') == $rClub->id)>
                    {{ $rClub->name }}
                </option>
            @endforeach
        </select>
        <x-input-error :messages="$errors->get('rival_id_2')" class="mt-1" />
    </div>
</div>

<div class="mt-4">
    <label
        class="inline-flex items-center gap-2 rounded-xl border border-slate-700/70 px-3 py-2 text-sm text-slate-200">
            ...
            <select class="sim-select" id="captain_player_id" name="captain_player_id">
                ...
                @foreach ($rolePlayers as $player)
                    <option value="{{ $player->id }}" @selected((string) old('captain_player_id', $club->captain_player_id ?? '') === (string) $player->id)>
                        {{ $player->full_name }} ({{ $player->position_main ?: $player->position }} | OVR
                        {{ $player->overall }})
                    </option>
                @endforeach
            </select>
