    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="flex flex-wrap gap-4 mb-2">
                <a href="{{ route('admin.monitoring.index') }}" class="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-xl shadow-lg shadow-indigo-500/20 font-bold transition text-sm">
                    <span>🏠</span> Übersicht
                </a>
                <a href="{{ route('admin.monitoring.analysis') }}" class="flex items-center gap-2 px-6 py-3 bg-slate-800 text-slate-300 rounded-xl hover:bg-slate-700 border border-slate-700 transition text-sm">
                ...
                <a href="{{ route('admin.monitoring.logs') }}" class="flex items-center gap-2 px-6 py-3 bg-slate-800 text-slate-300 rounded-xl hover:bg-slate-700 border border-slate-700 transition text-sm">
                    <span>📜</span> Logs
                </a>
            </div>

            @if (session('success'))
                <div class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-4 rounded-xl text-sm font-bold animate-fade-in">
                    ✅ {{ session('success') }}
                </div>
            @endif
            ...
            <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <!-- DB Status -->
                <article class="sim-card p-5 border-l-4 {{ $health['database'] ? 'border-l-emerald-500' : 'border-l-red-500' }}">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        @else
                            <span class="flex h-2 w-2 rounded-full bg-red-500 animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.8)]"></span>
                        @endif
                    </div>
                    <p class="text-xl font-bold text-white">{{ $health['database'] ? 'Bereit' : 'Verbindungsfehler' }}</p>
                    <p class="text-xs text-slate-400 mt-1">PDO Connection: ACTIVE</p>
                </article>

                <!-- Storage Status -->
                <article class="sim-card p-5 border-l-4 {{ $health['storage']['percentage'] < 90 ? 'border-l-cyan-500' : 'border-l-orange-500' }}">
                    ...
                    <div class="w-full bg-slate-700 h-1.5 rounded-full mt-3 overflow-hidden">
                        <div class="bg-cyan-400 h-full rounded-full" style="width: {{ $health['storage']['percentage'] }}%"></div>
                    </div>
                </article>

                <!-- Log Stats -->
                <article class="sim-card p-5 border-l-4 {{ $logStats['errors'] > 0 ? 'border-l-red-500' : 'border-l-emerald-500' }}">
                    <p class="sim-section-title mb-2">Fehlerrate (24h)</p>
                    <div class="flex items-center justify-between">
                        <p class="text-3xl font-bold {{ $logStats['errors'] > 0 ? 'text-red-400' : 'text-emerald-400' }}">{{ $logStats['errors'] }}</p>
                        <div class="text-right text-xs text-slate-400">
                            <p>Gesamt: {{ $logStats['total'] }}</p>
                            <p>Warnungen: {{ $logStats['warnings'] }}</p>
                        </div>
                    </div>
                    <p class="text-[10px] text-slate-500 mt-2 uppercase tracking-wider">Letzter Error: {{ $logStats['latest_error'] ?? 'Keiner' }}</p>
                </article>
                ...
                <article class="sim-card p-5 border-l-4 border-l-indigo-500">
                    ...
                    <div class="space-y-1">
                        <p class="text-xs text-white">PHP: <span class="text-indigo-300 font-mono">{{ $health['php_version'] }}</span></p>
                        <p class="text-xs text-white">Laravel: <span class="text-indigo-300 font-mono">{{ $health['laravel_version'] }}</span></p>
                        <p class="text-xs text-white">Env: <span class="text-indigo-300 font-mono uppercase">{{ $health['environment'] }}</span></p>
                        <p class="text-xs text-white">Debug: <span class="font-mono {{ $health['debug_mode'] ? 'text-emerald-400' : 'text-red-400' }}">{{ $health['debug_mode'] ? 'ON' : 'OFF' }}</span></p>
                    </div>
                </article>
            </section>
                ...
                <article class="sim-card p-5">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <div class="text-center">
                            ...
                            <p class="text-2xl font-bold text-cyan-400">{{ $diagnostics['performance']['avg_time_ms'] }}ms</p>
                        </div>
                        <div class="text-center">
                            <p class="text-[10px] text-slate-500 uppercase font-bold">Ø Aktionen / Spiel</p>
                            <p class="text-2xl font-bold text-indigo-400">{{ $diagnostics['performance']['avg_actions'] }}</p>
                        </div>
                    </div>
                ...
                <article class="sim-card p-5">
                    <div class="mb-5 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-slate-400">Pruefung wichtiger Datenbankfelder auf Vollstaendigkeit und Logik. <span class="text-cyan-400 ml-2">Stand: {{ $diagnostics['generated_at'] }}</span></p>
                        </div>
                        <div class="flex items-center gap-4">
                            <a href="{{ route('admin.monitoring.index', ['refresh' => 1]) }}" class="text-[10px] bg-slate-700 hover:bg-slate-600 text-slate-300 px-2 py-1 rounded transition-colors flex items-center gap-1">
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-lg font-bold text-white">{{ $diagnostics['matches']['count'] + $diagnostics['events']['count'] + $diagnostics['clubs']['count'] + $diagnostics['inactivity']['count'] }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 lg:grid-cols-2">
                        ...
                        <div class="space-y-3">
                            ...
                            @foreach(array_merge($diagnostics['matches']['issues'], $diagnostics['events']['issues']) as $issue)
                                <div class="sim-card-soft p-3 border-l-2 {{ $issue['severity'] === 'CRITICAL' ? 'border-l-red-500' : 'border-l-orange-500' }}">
                                    <div class="flex items-start justify-between gap-2">
                                        <div>
                                            <p class="text-xs font-bold text-white">{{ $issue['description'] }}</p>
                                            <p class="text-[10px] text-slate-400 mt-1"><span class="text-cyan-400 font-bold">Grund:</span> {{ $issue['reason'] }}</p>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                            @if(count($diagnostics['matches']['issues']) + count($diagnostics['events']['issues']) == 0)
                                <p class="text-xs text-slate-500 italic">Keine Probleme bei Spielen oder Ereignissen gefunden.</p>
                            @endif
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            @foreach(array_merge($diagnostics['clubs']['issues'], $diagnostics['inactivity']['issues']) as $issue)
                                <div class="sim-card-soft p-3 border-l-2 {{ $issue['severity'] === 'CRITICAL' ? 'border-l-red-500' : 'border-l-orange-500' }}">
                                    <div class="flex items-start justify-between gap-2">
                                        <div>
                                            <p class="text-xs font-bold text-white">@if(isset($issue['name'])) {{ $issue['name'] }}: @endif {{ $issue['description'] }}</p>
                                            <p class="text-[10px] text-slate-400 mt-1"><span class="text-cyan-400 font-bold">Grund:</span> {{ $issue['reason'] }}</p>
                                        </div>
                                        <div class="flex flex-col items-end gap-2">
                                            @if(str_contains($issue['description'], 'no active lineup'))
                                                <form action="{{ route('admin.monitoring.repair') }}" method="POST">
                                                    ...
                                                    <button type="submit" class="text-[10px] bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/40 px-2 py-1 rounded transition-colors flex items-center gap-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <section class="mt-6">
                <article class="sim-card">
                    <div class="p-5 border-b border-white/5 flex items-center justify-between">
                        ...
                        <a href="{{ route('admin.monitoring.logs') }}" class="sim-btn-muted text-xs">Vollstaendige Logs</a>
                    </div>

                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-800/30 text-[10px] uppercase font-bold text-slate-500 tracking-widest">
                            ...
                            <tbody class="divide-y divide-white/5">
                                @forelse($recentLogs as $log)
                                    <tr class="hover:bg-white/5 transition-colors group">
                                        <td class="px-5 py-3 text-xs font-mono text-slate-400 whitespace-nowrap">{{ $log['timestamp'] }}</td>
                                        <td class="px-5 py-3 text-xs">
                                            <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-tighter {{ $log['level'] === 'ERROR' || $log['level'] === 'CRITICAL' ? 'bg-red-500/20 text-red-400' : ($log['level'] === 'WARNING' ? 'bg-orange-500/20 text-orange-400' : 'bg-slate-700 text-slate-300') }}">
                                                {{ $log['level'] }}
                                            </span>
                                        </td>
                                        <td class="px-5 py-3 text-xs text-slate-200">
                                            <div class="line-clamp-1 group-hover:line-clamp-none transition-all cursor-help max-w-xl" title="{{ $log['message'] }}">
                                                {{ $log['message'] }}
                                            </div>
                                        </td>
                                        <td class="px-5 py-3 text-right">
                                            <button class="text-cyan-400 hover:text-white transition-colors Opacity-0 group-hover:opacity-100" onclick="alert('{{ addslashes($log['message']) }}')">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @empty
                                    <tr><td colspan="4" class="px-5 py-10 text-center text-slate-500 italic">Keine Logs gefunden.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
