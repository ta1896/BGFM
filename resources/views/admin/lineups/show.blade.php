<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <a href="{{ route('admin.lineups.edit', $lineup) }}" class="sim-btn-muted">Bearbeiten</a>
        </div>
    </x-slot>
    ...
    <section class="sim-card p-6">
        <div class="mb-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    <form method="POST" action="{{ route('admin.lineups.activate', $lineup) }}">
                        @csrf
                        <button class="sim-btn-muted" type="submit">Aktivieren</button>
                    </form>
                @endif
            </div>
        </div>
        <div class="grid gap-3 md:grid-cols-2">
            ...
            @empty
                <p class="text-sm text-slate-300">Keine Spieler zugewiesen.</p>
            @endforelse
        </div>
    </section>

    <form method="POST" action="{{ route('admin.lineups.destroy', $lineup) }}" class="sim-card border-rose-400/30 bg-rose-500/10 p-5">
        @csrf
        @method('DELETE')
        <div class="flex flex-wrap items-center justify-between gap-3">
            <p class="text-sm text-rose-100/90">Aufstellung loeschen</p>
