    ...
    private function createPlayers(Club $club, int $playersPerClub): Collection
    {
        ...
        for ($i = 0; $i < $playersPerClub; $i++) {
            ...
            $player = Player::create([
                ...
                'preferred_foot' => $i % 7 === 0 ? 'left' : ($i % 11 === 0 ? 'both' : 'right'),
                'height' => 170 + (($i * 7) % 32),
                'age' => 18 + ($i % 15),
                'overall' => $baseOverall,
                'potential' => min(99, $baseOverall + 8),
                'attr_attacking' => min(99, 52 + (($i * 5) % 38)),
                'attr_technical' => min(99, 54 + (($i * 4) % 34)),
                'attr_tactical' => min(99, 55 + (($i * 3) % 35)),
                'attr_defending' => min(99, 50 + (($i * 6) % 35)),
                'attr_creativity' => min(99, 53 + (($i * 2) % 33)),
                'happiness' => min(100, 58 + ($i % 30)),
                'sharpness' => min(100, 62 + (($i * 3) % 30)),
                'fatigue' => 0,
                'status' => 'active',
                ...
                'birthday' => now()->subYears(18 + ($i % 15))->subDays($i * 10),
